...
public class MainLogic {
	...
	public class Player_Action{
		static void New_Card() {
			if(GameRule.Player.Points<=(GameRule.Game.MaxGamePoints)) {
				...
				System.out.println(Lookup.BlackJack.Lang.Text.PlayerCards+" "+GameRule.Player.Points+" "+Lookup.BlackJack.Lang.Text.Points);
				}else {System.out.println(Lookup.BlackJack.Lang.Text.Not_Allowed);}
		}
	}
	public class cards {
	...
	public static void shuffle() {
		...
		for(int i=0; i<512; i++) {
			...
			temp=cards.card_deck[a];temp_int=cards.card_value[a];
			cards.card_deck[a]=cards.card_deck[b];cards.card_value[a]=cards.card_value[b];
			cards.card_deck[b]=temp;cards.card_value[b]=temp_int;
		}
	}
	...
	public static void play() {
		if(GameRule.Game.Save==true) {System.out.println(Lookup.BlackJack.Lang.Text.NoSavedResult);}else {System.out.println(Lookup.BlackJack.Lang.Text.SavedResult);}
		shuffle();
		int stop=1;
		int temp_int=0, a, score=0;
		int dealer_points=0, player_points=0;
		do {
			dealer_points=0; player_points=0;
			//dealer's hand
			...
			for(int i =0;i<1;i++) {
				...
				temp_int=temp_int+1;}
			System.out.println(Lookup.BlackJack.Lang.Text.DealerHas+" "+dealer_points+" "+Lookup.BlackJack.Lang.Text.Points);
			//player's hand
			...
			for(int i =0;i<2;i++) {
				...
				temp_int=temp_int+1;}
			System.out.println(Lookup.BlackJack.Lang.Text.YouHave+" "+player_points+" "+Lookup.BlackJack.Lang.Text.Points);
			a=1;//game logic
			while(a==1) {
			...
			while(dealer_points<GameRule.Dealer.WhenStop) {
				...
				dealer_points=dealer_points+cards.card_value[temp_int];
				temp_int=temp_int+1;
			}if(dealer_points>GameRule.Game.MaxGamePoints) {System.out.println(Lookup.BlackJack.Lang.Text.Player_Wins+"\n\n");score=score+1;}
			else {
				if(dealer_points>=player_points) { System.out.println(Lookup.BlackJack.Lang.Text.Dealer_Wins+"\n\n");score=score-1;}
				else { System.out.println(Lookup.BlackJack.Lang.Text.Player_Wins+"\n\n");score=score+1;}
			}
			}else {System.out.println(Lookup.BlackJack.Lang.Text.Dealer_Wins+"\n\n");score=score-1;}
